...
elseif ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link', array(
        'href' => 'order.php?act=order_query',
        'text' => $_LANG['03_order_query']
    ));
...
elseif ($_REQUEST['act'] == 'list2') {
    ...
    admin_priv('order_view');
    /* 模板赋值 */
    $smarty->assign('ur_here', $_LANG['02_order_list2']);
    $smarty->assign('action_link', array(
        'href' => 'order.php?act=order_query',
        'text' => $_LANG['03_order_query']
    ));
	...
	foreach($order_list['orders'] as $key=>$value){
		$order_list['orders'][$key]['province'] = get_region_name($value['province']);
		$order_list['orders'][$key]['city'] = get_region_name($value['city']);
		$order_list['orders'][$key]['district'] = get_region_name($value['district']);
	}	
...
elseif ($_REQUEST['act'] == 'query') {
    ...
    make_json_result($smarty->fetch('order_list.htm') , '', array(
...
elseif ($_REQUEST['act'] == 'info') {
	...
	/* 获取物流信息 */
	$wuliu_no=$order['shipping_name']." ".$order['invoice_no'];
	$wuliu_info=get_wuliu($wuliu_no);
	if(is_array($wuliu_info)){
		$smarty->assign('wuliu_type',$wuliu_info[1]);
		$smarty->assign('wuliu_info',$wuliu_info[0]);
	}else{
	//var_dump($wuliu_info);
	//exit;
		$smarty->assign('wuliu_info',$wuliu_info);
	}	
    ...
    elseif (isset($_GET['shipping_print'])) {
        ...
        if ($shipping['print_model'] == 2) {
			...
			//收件所在地址
			$region_array2 = array();
			$region_id2 = !empty($order['country']) ? $order['country'] . ',' : '';
			$region_id2.= !empty($order['province']) ? $order['province'] . ',' : '';
			$region_id2.= !empty($order['city']) ? $order['city'] . ',' : '';
			$region_id2.= !empty($order['district']) ? $order['district'] . ',' : '';
			$region_id2 = substr($region_id2, 0, -1);
			$region2 = $db->getAll("SELECT region_id, region_name FROM " . $ecs->table("region") . " WHERE region_id IN ($region_id2)");
			
			if (!empty($region2)) {
				foreach ($region2 as $region_data2) {
					$region_array2[$region_data2['region_id']] = $region_data2['region_name'];
				}
			}
			//var_dump($order);
            ...
            $lable_box['t_customer_country'] = $region_array2[$order['country']]; //收件人-国家
            $lable_box['t_customer_province'] = $region_array2[$order['province']]; //收件人-省份
            $lable_box['t_customer_city'] = $region_array2[$order['city']]; //收件人-城市
            $lable_box['t_customer_district'] = $region_array2[$order['district']]; //收件人-区/县
        ...
        $smarty->assign('action_link', array(
            'href' => 'order.php?act=list&' . list_link_postfix() ,
            'text' => $_LANG['02_order_list']
        ));
        /* 显示模板 */
        assign_query_info();
...
elseif ($_REQUEST['act'] == 'delivery_query') {
    ...
    $smarty->assign($sort_flag['tag'], $sort_flag['img']);
    make_json_result($smarty->fetch('delivery_list.htm') , '', array(
...
elseif ($_REQUEST['act'] == 'delivery_info') {
    ...
    $smarty->assign('action_link', array(
        'href' => 'order.php?act=delivery_list&' . list_link_postfix() ,
        'text' => $_LANG['09_delivery_order']
    ));
...
elseif ($_REQUEST['act'] == 'delivery_ship') {
    ...
    $links[] = array(
        'text' => $_LANG['09_delivery_order'],
        'href' => 'order.php?act=delivery_list'
    );
    $links[] = array(
        'text' => $_LANG['delivery_sn'] . $_LANG['detail'],
        'href' => 'order.php?act=delivery_info&delivery_id=' . $delivery_id
...
elseif ($_REQUEST['act'] == 'back_query') {
    ...
    $sort_flag = sort_flag($result['filter']);
    $smarty->assign($sort_flag['tag'], $sort_flag['img']);
    make_json_result($smarty->fetch('back_list.htm') , '', array(
        'filter' => $result['filter'],
        'page_count' => $result['page_count']
    ));
}
...
elseif ($_REQUEST['act'] == 'back_info') {
    ...
    /* 取得区域名 */
    $sql = "SELECT concat(IFNULL(c.region_name, ''), '  ', IFNULL(p.region_name, ''), " . "'  ', IFNULL(t.region_name, ''), '  ', IFNULL(d.region_name, '')) AS region " . "FROM " . $ecs->table('order_info') . " AS o " . "LEFT JOIN " . $ecs->table('region') . " AS c ON o.country = c.region_id " . "LEFT JOIN " . $ecs->table('region') . " AS p ON o.province = p.region_id " . "LEFT JOIN " . $ecs->table('region') . " AS t ON o.city = t.region_id " . "LEFT JOIN " . $ecs->table('region') . " AS d ON o.district = d.region_id " . "WHERE o.order_id = '" . $back_order['order_id'] . "'";
    ...
    $smarty->assign('action_link', array(
        'href' => 'order.php?act=back_list&' . list_link_postfix() ,
        'text' => $_LANG['10_back_order']
    ));
    assign_query_info();
...
elseif ($_REQUEST['act'] == 'step_post') {
        ...
        if (isset($_POST['rec_id'])) {
            foreach ($_POST['rec_id'] AS $key => $rec_id) {
                ...
                if ($goods_number_all >= $goods_number) {
                    /* 修改 */
                    $sql = "UPDATE " . $ecs->table('order_goods') . " SET goods_price = '$goods_price', " . "goods_number = '$goods_number', " . "goods_attr = '$goods_attr' " . "WHERE rec_id = '$rec_id' LIMIT 1";
                    $db->query($sql);
                } else {
    ...
    elseif ('add_goods' == $step) {
        ...
        if (is_spec($goods_attr) && !empty($prod)) {
            /* 插入订单商品 */
            $sql = "INSERT INTO " . $ecs->table('order_goods') . "(order_id, goods_id, goods_name, goods_sn,seller_note, product_id, goods_number, market_price, " . "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id) " . "SELECT '$order_id', goods_id, goods_name, goods_sn,seller_note, " . $product_info['product_id'] . ", " . "'$goods_number', market_price, '$goods_price', '" . $attr_value . "', " . "is_real, extension_code, 0, 0 , '" . implode(',', $goods_attr) . "' " . "FROM " . $ecs->table('goods') . " WHERE goods_id = '$goods_id' LIMIT 1";
        } else {
            $sql = "INSERT INTO " . $ecs->table('order_goods') . " (order_id, goods_id, goods_name, goods_sn,seller_note, " . "goods_number, market_price, goods_price, goods_attr, " . "is_real, extension_code, parent_id, is_gift)" . "SELECT '$order_id', goods_id, goods_name, goods_sn,seller_note, " . "'$goods_number', market_price, '$goods_price', '" . $attr_value . "', " . "is_real, extension_code, 0, 0 " . "FROM " . $ecs->table('goods') . " WHERE goods_id = '$goods_id' LIMIT 1";
        }
        $db->query($sql);
    ...
    elseif ('consignee' == $step) {
        /* 保存订单 */
        $order = $_POST;
        $order['agency_id'] = get_agency_by_regions(array(
            $order['country'],
            $order['province'],
            $order['city'],
            $order['district']
        ...
        if (isset($_POST['next'])) {
            ...
            if ('edit' == $step_act && exist_real_goods($order_id)) {
                ...
                if (!$exist) {
                    ...
                    $links[] = array(
                        'text' => $_LANG['step']['shipping'],
                        'href' => 'order.php?act=edit&order_id=' . $order_id . '&step=shipping'
                    );
        ...
        if (isset($_POST['next'])) {
            ...
            if ('edit' == $step_act && $shipping['support_cod'] == 0) {
                ...
                if ($payment['is_cod'] == 1) {
                    ...
                    $links[] = array(
                        'text' => $_LANG['step']['payment'],
                        'href' => 'order.php?act=' . $step_act . '&order_id=' . $order_id . '&step=payment'
                    );
                }
            }
            /* 显示提示信息 */
...
elseif ($_REQUEST['act'] == 'process') {
    ...
    elseif ('refund' == $func) {
        ...
        update_order($order_id, array(
            'order_amount' => 0,
            'money_paid' => $order['money_paid'] - $refund_amount
        ));
        /* 返回订单详情 */
        ecs_header("Location: order.php?act=info&order_id=" . $order_id . "\n");
        exit;
...
elseif ($_REQUEST['act'] == 'merge') {
    ...
    /* 取得满足条件的订单 */
    $sql = "SELECT o.order_sn, u.user_name " . "FROM " . $ecs->table('order_info') . " AS o " . "LEFT JOIN " . $ecs->table('users') . " AS u ON o.user_id = u.user_id " . "WHERE o.user_id > 0 " . "AND o.extension_code = '' " . order_query_sql('unprocessed');
...
elseif ($_REQUEST['act'] == 'operate') {
    ...
    elseif (isset($_REQUEST['remove_invoice'])) {
        ...
        $delivery_id = is_array($delivery_id) ? $delivery_id : array(
            $delivery_id
        );
    ...
    elseif (isset($_POST['print'])) {
        ...
        foreach ($order_sn_list as $order_sn) {
            ...
            /* 取得区域名 */
            $sql = "SELECT c.region_name as r_country, p.region_name as r_province, t.region_name as r_city, d.region_name as r_district " . "FROM " . $ecs->table('order_info') . " AS o " . "LEFT JOIN " . $ecs->table('region') . " AS c ON o.country = c.region_id " . "LEFT JOIN " . $ecs->table('region') . " AS p ON o.province = p.region_id " . "LEFT JOIN " . $ecs->table('region') . " AS t ON o.city = t.region_id " . "LEFT JOIN " . $ecs->table('region') . " AS d ON o.district = d.region_id " . "WHERE o.order_id = '$order[order_id]'";
            $order['region'] = $db->getRow($sql);
			$order['region'] = $order['region']['r_country']." ".$order['region']['r_province']." ".$order['region']['r_city']." ".$order['region']['r_district'];
            /* 其他处理 */
            $order['order_time'] = local_date($_CFG['time_format'], $order['add_time']);
            $order['pay_time'] = $order['pay_time'] > 0 ? local_date($_CFG['time_format'], $order['pay_time']) : $_LANG['ps'][PS_UNPAYED];
            $order['shipping_time'] = $order['shipping_time'] > 0 ? local_date($_CFG['time_format'], $order['shipping_time']) : $_LANG['ss'][SS_UNSHIPPED];
			...
			$sql = "SELECT tname FROM " . $ecs->table('users') . " WHERE user_id = '$order[user_id]'";
			$order['tname'] = $db->getOne($sql);
            /* 参数赋值：订单 */
			//var_dump($order);
            $smarty->assign('order', $order);
            /* 取得订单商品 */
...
elseif ($_REQUEST['act'] == 'operate_post') {
    ...
    elseif ('split' == $operation) {
        ...
        if ($order['order_status'] == OS_SPLITED) {
            ...
            $links[] = array(
                'text' => $_LANG['order_info'],
                'href' => 'order.php?act=info&order_id=' . $order_id
            );
        ...
        foreach ($goods_list as $key => $value) {
            ...
            elseif ($value['extension_code'] == 'virtual_card' || $value['is_real'] == 0) {
                ...
                if ($value['extension_code'] == 'virtual_card') {
                    $virtual_goods[$value['extension_code']][] = array(
                        'goods_id' => $value['goods_id'],
                        'goods_name' => $value['goods_name'],
                        'num' => $send_number[$value['rec_id']]
                    );
                }
            }
            // 商品（实货）、（货品）
                    ...
                    elseif ($value['extension_code'] == 'package_buy') {
                        foreach ($value['package_goods_list'] as $pg_key => $pg_value) {
                            $delivery_pg_goods = array(
								...
								'seller_note' => $pg_value['seller_note'],
    ...
    elseif ('return' == $operation) {
        ...
        if ($delivery_list) {
            foreach ($delivery_list as $list) {
                ...
                $sql_back_goods = "INSERT INTO " . $ecs->table('back_goods') . " (back_id, goods_id, product_id, product_sn, goods_name,goods_sn,seller_note, is_real, send_number, goods_attr)

                                   SELECT '$back_id', goods_id, product_id, product_sn, goods_name, goods_sn,seller_note, is_real, send_number, goods_attr
    ...
    if ($func == 'get_goods_info') {
        ...
        $sql = "SELECT goods_id, c.cat_name, goods_sn,seller_note, goods_name, b.brand_name, " . "goods_number, market_price, shop_price, promote_price, " . "promote_start_date, promote_end_date, goods_brief, goods_type, is_promote " . "FROM " . $ecs->table('goods') . " AS g " . "LEFT JOIN " . $ecs->table('brand') . " AS b ON g.brand_id = b.brand_id " . "LEFT JOIN " . $ecs->table('category') . " AS c ON g.cat_id = c.cat_id " . " WHERE goods_id = '$goods_id'";
        $goods = $db->getRow($sql);
        $today = gmtime();
        ...
        /* 取得商品属性 */
        $sql = "SELECT a.attr_id, a.attr_name, g.goods_attr_id, g.attr_value, g.attr_price, a.attr_input_type, a.attr_type " . "FROM " . $ecs->table('goods_attr') . " AS g, " . $ecs->table('attribute') . " AS a " . "WHERE g.attr_id = a.attr_id " . "AND g.goods_id = '$goods_id' ";
...
elseif ($_REQUEST['act'] == 'search_goods') {
    ...
    if ($keyword != '') {
        $sql = "SELECT goods_id, goods_name, goods_sn FROM " . $GLOBALS['ecs']->table('goods') . " WHERE is_delete = 0" . " AND is_on_sale = 1" . " AND is_alone_sale = 1" . " AND (goods_id LIKE '%" . mysql_like_quote($keyword) . "%'" . " OR goods_name LIKE '%" . mysql_like_quote($keyword) . "%'" . " OR goods_sn LIKE '%" . mysql_like_quote($keyword) . "%')" . " LIMIT 20";
        $res = $GLOBALS['db']->query($sql);
        $result['goodslist'] = array();
        while ($row = $GLOBALS['db']->fetchRow($res)) {
            $result['goodslist'][] = array(
                'goods_id' => $row['goods_id'],
                'name' => $row['goods_id'] . '  ' . $row['goods_name'] . '  ' . $row['goods_sn']
            );
        }
    } else {
        $result['error'] = 1;
...
elseif ($_REQUEST['act'] == 'export_act')
{
	/* 检查权限 */
	admin_priv('order_view');
	$sql_where = " 1 ";
	if($_REQUEST['order_status'] != "-1")
	{
		switch($_REQUEST['order_status'])
		 {
				case CS_AWAIT_PAY :
					$sql_where .= order_query_sql('await_pay');
					break;

				case CS_AWAIT_SHIP :
					$sql_where .= order_query_sql('await_ship');
					break;

				case CS_FINISHED :
					$sql_where .= order_query_sql('finished');
					break;

				case PS_PAYING :
					if ($_REQUEST['order_status'] != -1)
					{
						$sql_where .= " AND o.pay_status = '$_REQUEST[order_status]' ";
					}
					break;
				...
				default:
					if ($_REQUEST['order_status'] != -1)
					{
						$sql_where .= " AND o.order_status = '$_REQUEST[order_status]' ";
					}
			}
	}
	if($_REQUEST['start_time'])
	{
		...
		$sql_where .=" add_time >=  '$start_time' ";
	}
	if($_REQUEST['end_time'])
	{
		$sql_where.= $sql_where ? " and " : " ";
		$end_time = strtotime($_REQUEST['end_time']);
		$sql_where .=" add_time <=  '$end_time' ";
	}
	if($_REQUEST['order_id_start'])
	{
		$sql_where.= $sql_where ? " and " : " ";
		$sql_where.=" order_sn >=  '$_REQUEST[order_id_start]' ";
	}
	if($_REQUEST['order_id_end'])
	{
		$sql_where.= $sql_where ? " and " : " ";
		$sql_where.=" order_sn <=  '$_REQUEST[order_id_end]' ";
	}
	$sql_where = $sql_where ? " where ". $sql_where : " ";	

	$sql="select o.order_id, o.order_sn,o.consignee,o.province,o.city,o.district,o.address,o.mobile,o.add_time,o.shipping_name,o.pay_name,o.user_id,o.goods_amount,o.money_paid,o.surplus,o.own_money,o.divi_money,o.confirm_time,o.shipping_name,o.shipping_time,o.invoice_no, g.goods_id, g.goods_sn, g.goods_name,g.goods_attr, g.market_price, g.goods_price, g.goods_number,u.user_name,u.tname,u.user_rank,u.mobile_phone from  ". $GLOBALS['ecs']->table('order_info').
	 " as o left join " . $GLOBALS['ecs']->table('users')." as u on o.user_id=u.user_id "."left join  ". $GLOBALS['ecs']->table('order_goods')." as g on o.order_id=g.order_id $sql_where ";
	$res=$db->getAll($sql);
	foreach($res as $k=>$v){
		$res[$k]['rank_name'] = get_user_rank_info($v['user_id'],2);
		$res[$k]['province'] = get_region_name($v['province']);
		$res[$k]['city'] = get_region_name($v['city']);
		$res[$k]['district'] = get_region_name($v['district']);
		$ab_result = get_op_info($v['order_id']);
		$res[$k]['good_cost'] = 0;
		$res[$k]['op_cost'] = 0;
		$res[$k]['other_cost'] = 0;
		$res[$k]['profit'] = 0;
		$res[$k]['profit_share'] = 0;
		$res[$k]['all_pay_money'] = 0;
		foreach($ab_result as $key=>$value){
			$res[$k]['good_cost']+=$value['good_cost'];
			$res[$k]['op_cost']+=$value['op_cost'];
			$res[$k]['other_cost']+=$value['other_cost'];
			$res[$k]['profit']+=$value['profit'];
			$res[$k]['profit_share']+=$value['profit_share'];
			$res[$k]['all_pay_money']=$value['all_pay_money'];
		}	
	}	
	
	//var_dump(strtotime($_REQUEST['start_time']));
	//exit;
	...
	$objPHPExcel->getActiveSheet()->mergeCells('N1:Q1');
	$objPHPExcel->getActiveSheet()->mergeCells('R1:W1');
	
	//设置背景色
	$objPHPExcel->getActiveSheet()->getStyle( 'A1:W2')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
	$objPHPExcel->getActiveSheet()->getStyle( 'A1:W1')->getFill()->getStartColor()->setARGB('00989998');
	$objPHPExcel->getActiveSheet()->getStyle( 'A2:W2')->getFill()->getStartColor()->setARGB('00dde0df');
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A1', '所属人信息');
	$objPHPExcel->getActiveSheet()->setCellValue('E1', '商品信息');
	$objPHPExcel->getActiveSheet()->setCellValue('G1', '支付金额');
	$objPHPExcel->getActiveSheet()->setCellValue('J1', '各项成本');
	$objPHPExcel->getActiveSheet()->setCellValue('N1', '收货人信息');
	$objPHPExcel->getActiveSheet()->setCellValue('R1', '其他信息');
	
    $objPHPExcel->getActiveSheet()->setCellValue('A2', '所属ID');  
    $objPHPExcel->getActiveSheet()->setCellValue('B2', '所属人');  
    $objPHPExcel->getActiveSheet()->setCellValue('C2', '类型');  
    $objPHPExcel->getActiveSheet()->setCellValue('D2', '电话');  
    $objPHPExcel->getActiveSheet()->setCellValue('E2', '订单号');  
    $objPHPExcel->getActiveSheet()->setCellValue('F2', '订单金额');  
    $objPHPExcel->getActiveSheet()->setCellValue('G2', '自有余额');  
    $objPHPExcel->getActiveSheet()->setCellValue('H2', '分红余额');  
    $objPHPExcel->getActiveSheet()->setCellValue('I2', '合计支付');  
    $objPHPExcel->getActiveSheet()->setCellValue('J2', '商品成本');  
    $objPHPExcel->getActiveSheet()->setCellValue('K2', '运营成本');  
    $objPHPExcel->getActiveSheet()->setCellValue('L2', '其他成本'); 
	$objPHPExcel->getActiveSheet()->setCellValue('M2', '分成');  
    $objPHPExcel->getActiveSheet()->setCellValue('N2', '收货人');  
    $objPHPExcel->getActiveSheet()->setCellValue('O2', '电话');  
    $objPHPExcel->getActiveSheet()->setCellValue('P2', '收货地区');  
    $objPHPExcel->getActiveSheet()->setCellValue('Q2', '详细地址');  
    $objPHPExcel->getActiveSheet()->setCellValue('R2', '合计');	
	$objPHPExcel->getActiveSheet()->setCellValue('S2', '纯利润');  
    $objPHPExcel->getActiveSheet()->setCellValue('T2', '售出日期');  
    $objPHPExcel->getActiveSheet()->setCellValue('U2', '发货时间');  
    $objPHPExcel->getActiveSheet()->setCellValue('V2', '快递');
	$objPHPExcel->getActiveSheet()->setCellValue('W2', '快递单号');
	
    /////////////////////// 
	$n = 3;
    foreach($res as $k=>$v){
		$objPHPExcel->getActiveSheet()->setCellValue('A' . ($k+3), $v['user_id']);
		$objPHPExcel->getActiveSheet()->setCellValue('B' . ($k+3), $v['tname']);  
        $objPHPExcel->getActiveSheet()->setCellValue('C' . ($k+3), $v['rank_name']);  
		$objPHPExcel->getActiveSheet()->setCellValue('D' . ($k+3), $v['mobile_phone']);
		$objPHPExcel->getActiveSheet()->setCellValue('E' . ($k+3), "'".$v['order_sn']);  
		$objPHPExcel->getActiveSheet()->setCellValue('F' . ($k+3), price_format($v['goods_amount'])); 
		$objPHPExcel->getActiveSheet()->setCellValue('G' . ($k+3), price_format($v['own_money']+$v['money_paid'])); 
        $objPHPExcel->getActiveSheet()->setCellValue('H' . ($k+3), price_format($v['divi_money']));  
        $objPHPExcel->getActiveSheet()->setCellValue('I' . ($k+3), price_format($v['surplus']+$v['money_paid'])); 
        $objPHPExcel->getActiveSheet()->setCellValue('J' . ($k+3), $v['good_cost']);  
        $objPHPExcel->getActiveSheet()->setCellValue('K' . ($k+3), $v['op_cost']);  
        $objPHPExcel->getActiveSheet()->setCellValue('L' . ($k+3), $v['other_cost']);
		$objPHPExcel->getActiveSheet()->setCellValue('M' . ($k+3), $v['profit_share']);
		$objPHPExcel->getActiveSheet()->setCellValue('N' . ($k+3), $v['consignee']);  
        $objPHPExcel->getActiveSheet()->setCellValue('O' . ($k+3), $v['mobile']);
		$objPHPExcel->getActiveSheet()->setCellValue('P' . ($k+3), $v['province'].'-'.$v['city'].'-'.$v['district']);  
        $objPHPExcel->getActiveSheet()->setCellValue('Q' . ($k+3), $v['address']);
		$objPHPExcel->getActiveSheet()->setCellValue('R' . ($k+3), $v['all_pay_money']);
		$objPHPExcel->getActiveSheet()->setCellValue('S' . ($k+3), $v['profit']);
		$objPHPExcel->getActiveSheet()->setCellValue('T' . ($k+3), local_date('m-d H:i', $v['confirm_time']));	
		$objPHPExcel->getActiveSheet()->setCellValue('U' . ($k+3), local_date('m-d H:i', $v['shipping_time']));  		
		$objPHPExcel->getActiveSheet()->setCellValue('V' . ($k+3), $v['shipping_name']);  
        $objPHPExcel->getActiveSheet()->setCellValue('W' . ($k+3), "'".$v['invoice_no']);
		$n++;
	}
	...
	$objPHPExcel->getActiveSheet()->getStyle('A1:W'.$n)->applyFromArray($styleArray);
	//水平居中  
    $objPHPExcel->getActiveSheet()->getStyle('A1:W'.$n)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::VERTICAL_CENTER);  
    //垂直居中  
    $objPHPExcel->getActiveSheet()->getStyle('A1:W'.$n)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER); 
	...
	$file_name = "orderinfo_".date("Y-m-d-H-i",strtotime($_REQUEST['start_time']))."_".date("Y-m-d-H-i",strtotime($_REQUEST['end_time'])).".xls";
	$url = getcwd();
...
function return_user_surplus_integral_bonus($order) {
    ...
    if ($order['user_id'] > 0 && $order['surplus'] > 0) {
        ...
        log_account_change9($order['user_id'], $surplus, 0, 0, 0, sprintf($GLOBALS['_LANG']['return_order_surplus'], $order['order_sn']),$order['order_sn']);
	...
	if ($order['user_id'] > 0 && $order['goldcoin'] > 0) {
        log_account_change2($order['user_id'], 0, 0, $order['goldcoin'], 0, 0, sprintf($GLOBALS['_LANG']['return_order_goldcoin'], $order['order_sn']));
    }
    if ($order['bonus_id'] > 0) {
...
function handle_order_money_change($order, &$msgs, &$links) {
    ...
    if ($order['pay_status'] == PS_PAYED || $order['pay_status'] == PS_PAYING) {
        ...
        if ($money_dues > 0) {
            ...
            $links[] = array(
                'text' => $GLOBALS['_LANG']['refund'],
                'href' => 'order.php?act=process&func=load_refund&anonymous=' . $anonymous . '&order_id=' . $order_id . '&refund_amount=' . abs($money_dues)
            );
        }
    }
}
...
function order_list() {
    ...
    if ($result === false) {
        ...
        /* 查询 */
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," . "o.pay_status, o.consignee,o.province,o.city,o.district, o.address, o.email, o.tel, o.mobile, o.extension_code, o.extension_id, " . "(" . order_amount_field('o.') . ") AS total_fee, " . "IFNULL(u.user_name, '" . $GLOBALS['_LANG']['anonymous'] . "') AS buyer " . " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " . " LEFT JOIN " . $GLOBALS['ecs']->table('users') . " AS u ON u.user_id=o.user_id " . $where . " ORDER BY $filter[sort_by] $filter[sort_order] " . " LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";
        foreach (array(
            'order_sn',
            'consignee',
            'email',
...
function get_package_goods_list($package_id) {
    $sql = "SELECT pg.goods_id, g.goods_name, (CASE WHEN pg.product_id > 0 THEN p.product_number ELSE g.goods_number END) AS goods_number, p.goods_attr, p.product_id, pg.goods_number AS
            ...
            order_goods_number, g.goods_sn, g.seller_note, g.is_real, p.product_sn
    ...
    //            g.goods_number, pg.goods_number AS order_goods_number, g.goods_sn, g.seller_note, g.is_real " .
    //            "FROM " . $GLOBALS['ecs']->table('package_goods') . " AS pg, " .
...
function get_wuliu($type){
	...
	if(mb_substr($type,0,2,"UTF-8")=="圆通"){
		 $url = 'http://www.kuaidi100.com/applyurl?key=c1a08fdd30e4ca59&com=yuantong&nu=' .$invoice_sn;		
		 $w_type='html'; 	 
	}elseif(mb_substr($type,0,2,"UTF-8")=="申通"){
		 $url = 'http://www.kuaidi100.com/applyurl?key=c1a08fdd30e4ca59&com=shentong&nu=' .$invoice_sn;
		 $w_type='html'; 		
	}elseif(mb_substr($type,0,2,"UTF-8")=="天天"){
		 //$url = 'http://api.kuaidi100.com/api?id=c1a08fdd30e4ca59&com=tiantian&nu=' .$invoice_sn. '&show=0&order=desc';	
		 //$w_type='api'; 	
		 $url = 'http://www.kuaidi100.com/applyurl?key=c1a08fdd30e4ca59&com=tiantian&nu=' .$invoice_sn;
		 $w_type='html'; 
	}
