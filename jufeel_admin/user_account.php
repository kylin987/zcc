...
if ($_REQUEST['act'] == 'list')
{
	...
	//判断是否为超级管理员
	$admin=0;
	if($_SESSION['action_list']=="all" || strpos($_SESSION['action_list'],'surplus_action') !== false){
		$admin=1;
	}
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    {		
		if(strpos($_SESSION['action_list'],'surplus_action') !== false){
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_paid == 1)
    {
        $change_desc = $amount > 0 ? "管理员".$_LANG['surplus_type_0'] : $_LANG['surplus_type_1'];
...
elseif ($_REQUEST['act'] == 'check')
{
    ...
    elseif ($account['process_type'] == 1)
    {
		...
		if($account['is_premium'] == 1){
			$smarty->assign('is_premium_name', "入股金");	
		}
		$smarty->assign('raply_info',      get_raply_info($account['user_id']));
    }
    ...
    if ($account['is_paid'] == 0)
    {
        ...
        if ($is_paid == '1' && $account['process_type'] == '1')
        {
			if($account['is_premium'] == 0){
				...
				//更新会员余额数量
				log_account_change($account['user_id'], $amount, 0,0, 0, 0, $_LANG['surplus_type_1'], ACT_DRAWING,2);
			}else{
				//操作入股金提现
				$user_frozen_premium = get_user_frozen_premium($account['user_id']);
				$fmt_amount   = str_replace('-', '', $amount);
				//如果扣除的余额多于此会员拥有的冻结入股金，提示
				if ($fmt_amount > $user_frozen_premium)
				{
					$link[] = array('text' => $_LANG['go_back'], 'href'=>'javascript:history.back(-1)');
					sys_msg($_LANG['surplus_amount_error'], 0, $link);
				}
				update_user_account($id, $amount, $admin_note, $is_paid);
				if($account['is_premium'] == '2'){
					$info = "入股金转余额";
					//更新会员冻结入股金数量及余额					
					log_account_change3($account['user_id'], $amount*-1, 0,0, 0, 0,$amount,$info , ACT_DRAWING);
					update_user_back_stock($account['s_id'],2);
				}else{
					$info = "入股金提现";
					//更新会员冻结入股金数量
					log_account_change3($account['user_id'], 0, 0,0, 0, 0,$amount,$info , ACT_DRAWING);
					update_user_back_stock($account['s_id'],1);
				}
        ...
        elseif ($is_paid == '1' && $account['process_type'] == '0')
        {
            ...
            //更新会员余额数量
            log_account_change($account['user_id'], $amount, 0, 0, 0, "管理员".$_LANG['surplus_type_0'], ACT_SAVING);
 ...
 * @return  int
 */
function get_user_gold($user_id)
{
    $sql = "SELECT SUM(gold_coin) FROM " .$GLOBALS['ecs']->table('account_log').
           " WHERE user_id = '$user_id'";
 ...
 * @return  int
 */
function get_user_frozen_premium($user_id)
{
    $sql = "SELECT frozen_premium FROM " .$GLOBALS['ecs']->table('users').
           " WHERE user_id = '$user_id'";
...
//获取用户提现银行信息
function get_raply_info($user_id){
	$sql = "SELECT raply_bank,raply_kaihu,raply_username,raply_number,raply_beizhu FROM " .$GLOBALS['ecs']->table('users').
           " WHERE user_id = '$user_id'";
...
//更新退股金后续操作
function update_user_back_stock($s_id, $do_type)
{
    $sql = "UPDATE " .$GLOBALS['ecs']->table('tg_log'). " SET ".
           "do_type  = '$do_type' WHERE s_id = '$s_id'";
    return $GLOBALS['db']->query($sql);
}
...
function account_list()
{
    ...
    if ($result === false)
    {
		...
		if (strstr($_REQUEST['start_date'], '-') === false)
		{
			$filter['start_date'] = local_date('Y-m-d', $_REQUEST['start_date']);
			$filter['end_date'] = local_date('Y-m-d', $_REQUEST['end_date']);
		}
        ...
        /* 查询数据 */
        $sql  = 'SELECT ua.*, u.user_name,u.tname FROM ' .
    ...
    foreach ($list AS $key => $value)
    {
		...
		$list[$key]['is_premium_name']    = $list[$key]['is_premium'] == 1?"入股金":"";
		if($list[$key]['is_premium'] == '2'){
			$list[$key]['process_type_name'] = "转余额";
			$list[$key]['is_premium_name'] = "入股金";
		}
     }
