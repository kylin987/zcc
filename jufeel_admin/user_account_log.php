...
if ($_REQUEST['act'] == 'list')
{
    ...
    $logdb = get_account_log_list();
	//var_dump($logdb);
	//exit;
    $smarty->assign('full_page',  1);
    $smarty->assign('ur_here', '用户流水明细');
    ...
    assign_query_info();
    $smarty->display('user_account_log_list.htm');
}
...
elseif (isset($_REQUEST['act']) && ($_REQUEST['act'] == 'query' ||  $_REQUEST['act'] == 'download'))
{
    ...
    if ($_REQUEST['act'] == 'download')
    {
        ...
        $logdb = get_account_log_list(false);
		//var_dump($logdb);
		//exit;
        /*------------------------------------------------------ */
		//--Excel文件下载
		...
		$objPHPExcel->getActiveSheet()->getStyle( 'A1:N1')->getFill()->getStartColor()->setARGB('00989998');
		$objPHPExcel->getActiveSheet()->getStyle( 'A2:N2')->getFill()->getStartColor()->setARGB('00dde0df');
		...
		$objPHPExcel->getActiveSheet()->setCellValue('C1', '变更人信息'); 
		$objPHPExcel->getActiveSheet()->setCellValue('G1', '资金变动');
		$objPHPExcel->getActiveSheet()->setCellValue('J1', '资金变动');
		...
		$objPHPExcel->getActiveSheet()->setCellValue('C2', '用户ID');  
		$objPHPExcel->getActiveSheet()->setCellValue('D2', '真实姓名');  
		$objPHPExcel->getActiveSheet()->setCellValue('E2', '电话');  
		$objPHPExcel->getActiveSheet()->setCellValue('F2', '用户等级');  
		
		$objPHPExcel->getActiveSheet()->setCellValue('G2', '自有余额');  
		$objPHPExcel->getActiveSheet()->setCellValue('H2', '分红余额');  
		$objPHPExcel->getActiveSheet()->setCellValue('I2', '总额'); 
		
		$objPHPExcel->getActiveSheet()->setCellValue('J2', '等级积分');  
		$objPHPExcel->getActiveSheet()->setCellValue('K2', '银币');  
		$objPHPExcel->getActiveSheet()->setCellValue('L2', '冻结入股金');

		$objPHPExcel->getActiveSheet()->setCellValue('M2', '类型'); 
		...
		foreach($logdb['logdb'] as $k=>$v){
			$objPHPExcel->getActiveSheet()->setCellValue('A' . ($k+3), $v['log_id']);
			$objPHPExcel->getActiveSheet()->setCellValue('B' . ($k+3), $v['change_time']);
			$objPHPExcel->getActiveSheet()->setCellValue('C' . ($k+3), $v['user_id']);  
			$objPHPExcel->getActiveSheet()->setCellValue('D' . ($k+3), $v['tname']);  
			$objPHPExcel->getActiveSheet()->setCellValue('E' . ($k+3), $v['mobile_phone']);
			$objPHPExcel->getActiveSheet()->setCellValue('F' . ($k+3), $v['rankname']);  
			$objPHPExcel->getActiveSheet()->setCellValue('G' . ($k+3), $v['own_money']); 
			$objPHPExcel->getActiveSheet()->setCellValue('H' . ($k+3), $v['divi_money']); 
			$objPHPExcel->getActiveSheet()->setCellValue('I' . ($k+3), $v['user_money']);  
			$objPHPExcel->getActiveSheet()->setCellValue('J' . ($k+3), $v['rank_points']); 
			$objPHPExcel->getActiveSheet()->setCellValue('K' . ($k+3), $v['pay_points']);  
			$objPHPExcel->getActiveSheet()->setCellValue('L' . ($k+3), $v['frozen_premium']);  
			$objPHPExcel->getActiveSheet()->setCellValue('M' . ($k+3), $_LANG['change_type'][$v['change_type']]);
			$objPHPExcel->getActiveSheet()->setCellValue('N' . ($k+3), $v['change_desc']);			 
		...
		$file_name = "mingxi_".date("Y-m-d-H-i",strtotime($_REQUEST['start_time']))."_".date("Y-m-d-H-i",strtotime($_REQUEST['end_time'])).".xls";
		//$url = getcwd();
    ...
    make_json_result($smarty->fetch('user_account_log_list.htm'), '', array('filter' => $logdb['filter'], 'page_count' => $logdb['page_count']));
}



function get_account_log_list($is_pagination = true)
{
    ...
    if (isset($_REQUEST['ky']))
    {
		switch ($_REQUEST['ky']){
			case 0:
			  $sqladd .= ' AND a.user_money <> 0';
			  break;
			case 1:
			  $sqladd .= ' AND a.rank_points <> 0';
			  break;  
			case 2:
			  $sqladd .= ' AND a.pay_points <> 0';
			  break;
			case 3:
			  $sqladd .= ' AND a.frozen_premium <> 0';
			  break;
			default:
	...
	$sql = "SELECT a.*,u.tname,u.user_rank,u.mobile_phone FROM " . $GLOBALS['ecs']->table('account_log') . " a".
