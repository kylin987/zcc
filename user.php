...
$not_login_arr =
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer','oath','oath_login','other_login','select_username');

/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'collection_list',
'message_list', 'tag_list','bdwechat','wechat_jb', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log', 'account_detail', 'act_account', 'coin_log', 'transfer_coin','transfer_surplus', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer');
...
if(isset($_REQUEST['code'])&&isset($_REQUEST['state'])){
	...
	$c = &website('wechat');
	if($c) {
		if(!wx_check_state($_REQUEST['state'])) {  //state判断方式，自己写方法
			show_message('非法访问或请求超时1！' , '首页',$ecs->url() , 'error');
		}		
		...
		//判断是绑定还是登陆。登陆的state前2位是lg，绑定的前2位是bd
		$sta2=substr($_REQUEST['state'],0,2);
		
		if($sta2=="lg"){	//如果是登陆		
			$info_user_id = 'wx_'.$info['unionid']; 
			if(!$info['openid'])
				show_message($c->get_error() , '首页' , $ecs->url() , 'error' , false);
			$sql = 'SELECT ect_uid,unionid FROM '.$ecs->table('wechat_user').' WHERE unionid = "'.$unionid.'"';
			$count = $db->getRow($sql);
			if(!$count){	//如果微信表没有unionid记录
				$sql = 'SELECT user_name,password,aite_id FROM '.$ecs->table('users').' WHERE aite_wx_id = "'.$info_user_id.'"';
				$count2 = $db->getRow($sql);
				if(!$count2){	//如果user表里也没有相关记录
					if($user->check_user($info['nickname']))  // 重名处理
					{
						$info['nickname'] = $info['nickname'].'_'.$type.(rand(10000,99999));
					}
					$user_pass = $user->compile_password(array('password'=>$info['openid']));
									
					$sql = 'INSERT INTO '.$ecs->table('users').'(user_name , password, aite_wx_id ,wxid,wx_headimg, sex , reg_time , user_rank , is_validated) VALUES '.
							"('$info[nickname]' , '$user_pass' , '$info_user_id' ,'$info[openid]','$info[headimgurl]', '$info[sex]' , '".gmtime()."' , '1' , '1')" ;
					$w_res=$db->query($sql);
					if($w_res){
						//新注册会员贡献点数和银币各加50
						$w_info="注册送积分";
						$sql = 'SELECT user_id FROM '.$ecs->table('users').' WHERE aite_wx_id = "'.$info_user_id.'"';
						$count3 = $db->getRow($sql);					
						log_account_change($count3['user_id'],0,0,10,10,$w_info);
					}
					/*推荐处理*/
					$affiliate  = unserialize($GLOBALS['_CFG']['affiliate']);
					if (isset($affiliate['on']) && $affiliate['on'] == 1)
					{
						...
						if ($up_uid)
						{
							...
							//设置推荐人
							$sql = 'UPDATE '. $GLOBALS['ecs']->table('users') . ' SET parent_id = ' . $up_uid . ' WHERE user_id = ' . $count3['user_id'];
				...
				$sql = 'SELECT user_name,password,aite_id,user_id FROM '.$ecs->table('users').' WHERE aite_wx_id = "'.$info_user_id.'"';
				$count2 = $db->getRow($sql);
				if(!$count2){	//如果只有微信表里有记录，user表里没有记录，自动绑定现有账户
					if($user->check_user($info['nickname']))  // 重名处理
					{
						$info['nickname'] = $info['nickname'].'_'.$type.(rand(10000,99999));
					}
					$sql = 'UPDATE '.$ecs->table('users')." SET user_name = '$info[nickname]',aite_wx_id = '$info_user_id',wxid = '$info[openid]',wx_headimg = '$info[headimgurl]',is_validated = '1' WHERE user_id = '$count[ect_uid]'";
					$db->query($sql);
					$user_name = $info['nickname'];			
				}else{	//如果微信表里有记录，user表里也有记录，那么分为2种情况。1、2表统一，即已经绑定。2、2表不一致，仅发生于先登录pc，后又第一次登录微信端（wechat里本来有资料），再次登录pc这种情况
					if($count['ect_uid']==$count2['user_id']){	//2表一致，正常
						$user_name = $count2['user_name'];
					}else{
						show_message('您拥有2个冲突的微信账户，请联系管理员进行合并！' , '首页',$ecs->url() , 'error');
					}
				}
			}		
			...
			$sql = 'SELECT ect_uid,unionid FROM '.$ecs->table('wechat_user').' WHERE unionid = "'.$unionid.'"';
			$count = $db->getRow($sql);
			if(!$count){	//如果微信表没有unionid记录,说明该微信号还未和任何账户绑定
				$sql = 'SELECT user_name,aite_wx_id FROM '.$ecs->table('users').' WHERE user_id = "'.$_SESSION[user_id].'"';
				$count2 = $db->getRow($sql);
				if(!$count2['aite_wx_id']){	//如果该账户没有aite_wx_id值，说明从未绑定过					
					if($user->check_user($info['nickname']))  // 重名处理
					{
						$info['nickname'] = $info['nickname'].'_'.$type.(rand(10000,99999));
					}
					$sql = 'UPDATE '.$ecs->table('users')." SET aite_wx_id = '$info_user_id',wxid = '$info[openid]',wx_headimg = '$info[headimgurl]',is_validated = '1' WHERE user_id = '$_SESSION[user_id]'";
					$db->query($sql);
					
					show_message('微信绑定成功！' , array($_LANG['back_up_page'], $_LANG['profile_lnk']), array('index.php','user.php'), 'info');
				}else{	//如果该账户有aite_wx_id值，说明已经和别的微信绑定
					show_message('该账户已绑定微信账户，请解绑后重试！' , '首页',$ecs->url() , 'error');
				}
			}else{		//如果微信表里有相关的记录，说明这个微信号已经和一个账户绑定了
				if($count['ect_uid']==$_SESSION['user_id']){
					$sql = 'UPDATE '.$ecs->table('users')." SET aite_wx_id = '$info_user_id',wxid = '$info[openid]',wx_headimg = '$info[headimgurl]',is_validated = '1' WHERE user_id = '$_SESSION[user_id]'";
					$db->query($sql);
					show_message('您已经和该微信绑定成功！' , array($_LANG['back_up_page'], $_LANG['profile_lnk']), array('index.php','user.php'), 'info');
				}else{
					$sql = 'SELECT user_id,user_name FROM '.$ecs->table('users').' WHERE user_id = "'.$count[ect_uid].'"';
					$count3 = $db->getRow($sql);
					$err_log="您的微信已和".$count3['user_name']."的用户名绑定,请尝试直接微信扫描登陆";
					show_message($err_log , '首页',$ecs->url() , 'error');
				}
			}			
		}else{
			show_message('非法访问或请求超时！' , '首页',$ecs->url() , 'error');
...
/* by kylin 绑定微信 */
elseif ($action == 'bdwechat')
{
	if($_SESSION['user_id']){
		$sql = 'SELECT user_name,aite_wx_id,wxid,wx_headimg FROM '.$ecs->table('users').' WHERE user_id = "'.$_SESSION[user_id].'"';
		$count = $db->getRow($sql);
		if($count['aite_wx_id']){
			$wx_headimg=$count['wx_headimg'];
			if(!$wx_headimg){
				$wx_headimg="/images/no_headimg.gif";				
			}
			$smarty->assign('wx_headimg',$wx_headimg);	
		}
	}
    $smarty->display('user_clips.dwt');
}
/* by kylin 解绑微信 */
elseif ($action == 'wechat_jb')
{
	if($_SESSION['user_id']){
		$sql = 'SELECT user_name,aite_wx_id,wxid,wx_headimg FROM '.$ecs->table('users').' WHERE user_id = "'.$_SESSION[user_id].'"';
		$count = $db->getRow($sql);
		if(!$count['aite_wx_id']){
			show_message('没有微信用户绑定该账户！' , '首页',$ecs->url() , 'error');
		}else{
			$sql = 'UPDATE '.$ecs->table('users')." SET aite_wx_id = '',wxid = '',wx_headimg = '',is_validated = '0' WHERE user_id = '$_SESSION[user_id]'";
			$db->query($sql);
			$sql = 'DELETE FROM '.$ecs->table('wechat_user')." WHERE ect_uid = '$_SESSION[user_id]'";
			$db->query($sql);
			show_message('解绑成功！' , array($_LANG['back_up_page'], $_LANG['profile_lnk']), array('index.php','user.php'), 'info');	
		}
	}
    $smarty->display('user_clips.dwt');
}
...
elseif ($action == 'default')
{
    ...
    if ($rank = get_rank_info())
    {
        ...
        $smarty->assign('rank_name', sprintf($_LANG['your_level'], $rank['rank_name']."(".$rank['user_rank']." 积分)"));
...
if ($action == 'build_qrcode') {
	...
	//$smarty->assign('qrcode', QRcode::png($data, false, "L", 4));
	exit;    
}
if ($action == 'show_qrcode') {   
	...
	$img_url='user.php?act=build_qrcode&data='.$data;
...
if ($action == 'get_res') {	
	...
	if($order_type == "0"){
		...
		$sql = "SELECT * FROM ". $ecs->table('pay_log'). "WHERE order_id='$order_sn'";
		$zfinfo = $db->getRow($sql);
		if($zfinfo['is_paid']==1){
			$result['status']=2;
			$result['type']=1;
	...
	if($c)
	{
		...
		if(!$count)   // 没有当前数据
		{
			...
			if($w_res){
				...
				$count3 = $db->getRow($sql);					
				log_account_change($count3['user_id'],0,0,50,50,$w_info);
			}
			/*推荐处理*/
			$affiliate  = unserialize($GLOBALS['_CFG']['affiliate']);
			if (isset($affiliate['on']) && $affiliate['on'] == 1)
...
elseif ($action == 'act_edit_profile')
{
	...
	$other['tname'] = $qq = isset($_POST['tname']) ? trim($_POST['tname']) : '';
...
elseif ($action == 'order_detail')
{
	...
	//获取相应地区ID的具体名字
	$order['country_name']=get_region_name($order['country']);
...
elseif ($action == 'save_order_address')
{
    ...
    $address = array(
		...
		'district'       => isset($_POST['district'])       ? compile_str(trim($_POST['district']))        : '',
        'zipcode'   => isset($_POST['zipcode'])   ? compile_str(make_semiangle(trim($_POST['zipcode']))) : '',
...
elseif ($action == 'coin_log')
{
    ...
    //获取剩余代金券
    $coin_amount = get_user_coin($user_id);
	...
	//var_dump($account_log);
    //模板赋值
    $smarty->assign('coin_amount', $coin_amount);
    $smarty->assign('account_log',    $account_log);
    $smarty->assign('pager',          $pager);
    $smarty->display('user_transaction.dwt');
}
...
/* 代金券转账 */
elseif ($action == 'transfer_coin')
{
    include_once(ROOT_PATH . 'includes/lib_clips.php');
	
	//获取剩余代金券
    $sliv_amount = get_user_coin($user_id);
	...
	if($_POST){
		$user_nicheng = get_user_nc_tx($user_id,2);
		//获取传输来的值
		...
		if(!filter_var($coin_number, FILTER_VALIDATE_INT,array('options' => array('min_range' => 0))) || empty($user_name)){
			...
			if((int)$coin_number > (int)$sliv_amount){
				...
				$user_info = get_user_tname($user_name);
				if($user_info['user_id'] == $user_id){
					show_message("不能给自己转账", '', '', 'info');
				}				
				if($user_info){
					//给接收人加钱
					$info = "来自用户".$user_nicheng."的转账";										
					$result = log_account_change8($user_info['user_id'],0,0,0,$coin_number,$info,19,1,$user_id);
					if($result){
						//给发送人减钱
						$info = "向用户".$user_name."转账";
						$result2 = log_account_change8($user_id,0,0,0,-$coin_number,$info,18,1,$user_info['user_id']);
						if($result2){
							show_message("转账成功", "查看详情", "user.php?act=coin_log", 'info');
						}else{
		...
		$smarty->assign('sliv_amount', $sliv_amount);
		$smarty->display('user_transaction.dwt');
	}
    
}
/* 余额转账 */
elseif ($action == 'transfer_surplus')
{
    include_once(ROOT_PATH . 'includes/lib_clips.php');
	...
	if($_POST){
		...
		if($surplus_number <= 0 || empty($user_name)){
			show_message("参数错误，请重新输入", '', '', 'info');
		}else{
			if($surplus_number > (float)$surplus_amount){
				...
				if ($user_info) {						
					...
					$result2 = log_account_change8($this->user_id, -$surplus_number, 0, 0, 0, $info,8,1,$user_info['user_id']);
					if ($result2) {
						//给接收人加钱
						$info = "来自用户" . $user_nicheng . "的余额转账";
						$result = log_account_change8($user_info['user_id'], $surplus_number, 0, 0, 0, $info,9,1,$this->user_id,$result2);
						if ($result) {
...
elseif ($action == 'select_username'){
	...
	$user_tname = get_user_tname($user_name);
	
	$result = array('error' => '', 'content' => '');
	if($user_tname){    
		if(empty($user_tname['tname'])){
			$result['error'] ="red";
			$result['content'] ="未核实到真实姓名，请谨慎转账";	
		}else{
			$result['error'] ="green";
			$result['content'] ="对方真实姓名：".$user_tname['tname'];	
		}
	}else{
		$result['error'] ="red";
