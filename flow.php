...
elseif ($_REQUEST['step'] == 'checkout')
{
        ...
        && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS))
    {
		...
		$smarty->assign('order_min_integral', flow_available_points2());  // 至少需要用积分
...
elseif ($_REQUEST['step'] == 'change_integral')
{
	...
	$flow_min_points = flow_available_points2();  // 该订单允许使用的积分
    ...
    {
        $result['error'] = sprintf($_LANG['integral_too_less'], $flow_min_points);
...
elseif ($_REQUEST['step'] == 'check_integral')
{
    ...
    $user_info   = user_info($_SESSION['user_id']);
    $flow_points = flow_available_points();  // 该订单允许使用的积分
	$flow_min_points = flow_available_points2();  // 该订单必须使用的积分
    $user_points = $user_info['pay_points']; // 用户的积分总数
    ...
    {
        die(sprintf($_LANG['integral_too_less'], $flow_min_points));
    }
 ...
 * @return  integral
 */
function flow_available_points2()
{
    $sql = "SELECT SUM(g.integral * c.goods_number) ".
            "FROM " . $GLOBALS['ecs']->table('cart') . " AS c, " . $GLOBALS['ecs']->table('goods') . " AS g " .
            "WHERE c.session_id = '" . SESS_ID . "' AND c.goods_id = g.goods_id AND c.is_gift = 0 AND g.integral > 0 AND g.is_static = 1 " .
            "AND c.rec_type = '" . CART_GENERAL_GOODS . "'";
