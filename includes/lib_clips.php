 ...
 * @return  array
 */
function get_account_log2($user_id, $num, $start)
{
    $account_log = array();
    $sql = 'SELECT * FROM ' .$GLOBALS['ecs']->table('user_account').
           " WHERE user_id = '$user_id'" .
...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT pay_points, user_money,frozen_money, gold_coin, credit_line, last_login, is_validated FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
	...
	$info['gold_coin'] = $row['gold_coin']."枚";
...
function get_rank_info()
{
    ...
    if (!empty($_SESSION['user_rank']))
    {
        ...
        else
        {
            ...
            return array('rank_name'=>$rank_name,'next_rank_name'=>$next_rank_name,'next_rank'=>$next_rank,'user_rank'=>$user_rank);
        }
    }
    else
    {
