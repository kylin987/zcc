...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order)
    {
		...
		$order['own_money'] = $order['own_money']-$order['money_paid'];
		$order['formated_own_money']        = price_format($order['own_money'], false);
		$order['formated_divi_money']        = price_format($order['divi_money'], false);
		$order['formated_goldcoin']        = price_format($order['goldcoin'], false);
...
function cart_goods($type = CART_GENERAL_GOODS)
{
    ...
    foreach ($arr as $key => $value)
    {
		...
		$arr[$key]['goods_thumb']     = get_goods_thumb($value['goods_id']);
...
//        }
		...
		$user['formated_gold_coin'] = price_format($user['gold_coin'], false);
    }
 ...
 * @return  void
 */
function log_account_change4($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER)
{
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'       => $user_id,
