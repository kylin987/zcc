...
function log_account_change3($user_id, $user_money = 0, $frozen_money = 0, $gold_coin = 0, $rank_points = 0, $pay_points = 0,$frozen_premium = 0, $change_desc = '', $change_type = ACT_OTHER)
{
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'       => $user_id,
    ...
    $sql = "UPDATE " . $GLOBALS['ecs']->table('users') .
            " SET user_money = user_money + ('$user_money')," .
            " own_money = own_money + ('$user_money')," .
            " frozen_money = frozen_money + ('$frozen_money')," .			
			" gold_coin = gold_coin + ('$gold_coin')," .
            " rank_points = rank_points + ('$rank_points')," .
            " pay_points = pay_points + ('$pay_points')," .
			" frozen_premium = frozen_premium + ('$frozen_premium')" .
            " WHERE user_id = '$user_id' LIMIT 1";
    $GLOBALS['db']->query($sql);
...
function log_account_change8($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER,$ky_type = '',$other_userid = '',$bef_logid = '')
{
	...
	$log_id = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('account_log'), $account_log, 'INSERT');
	/* 更新用户信息 */
    $sql = "UPDATE " . $GLOBALS['ecs']->table('users') .
            " SET user_money = user_money + ('$user_money')," .
			$k_sql . 
...
//取消订单专用log
	function log_account_change9($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '',$order_id = '') {
		...
		foreach($res as $k=>$v){
			...
			/* 更新用户信息 */
			$sql = "UPDATE " . $GLOBALS['ecs']->table('users') .
					" SET user_money = user_money - ('$v[user_money]')," .
...
//插入到ab_log表里
	function log_ab_change($order_id,$unpay = 0) {		
		//根据order_id获得产品数量、所需代金券、价格
		$goods_info = get_goods_info2($order_id);
		if($goods_info)	{
			//获取分成的情况
			...
			foreach($goods_info as $k=>$v){
				...
				if($unpay == 1){
					$ab['profit'] = $ab['profit']*-1;
					$ab['note'] = "取消订单";					
				}
				
				
				$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('ab_log'), $ab, 'INSERT');
			}
		}
	}
	
//根据order_id获得产品数量、所需代金券、价格
	function get_goods_info2($order_id){		
...
//更新用户任务明细
function log_task($order_id,$unpay = 0){
	//根据order_id获得产品数量、所需代金券、价格
	$user_order_info = get_order_and_user_info($order_id);
	//logg($user_order_info);
	if($user_order_info['order_amount'] == '0.00'){		
		...
		if($user_order_info['user_rank'] == '138' || $user_order_info['user_rank'] == '144'){			
			...
			if($unpay == 1){
				$task['task_value'] = $task_value*-1;
			}
			$task['order_sn'] = $user_order_info['order_sn'];
			$task['is_self'] = 1;
			$task['change_time'] = gmtime();
			logg($task);
			$t_id = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('task_log'), $task, 'INSERT');
			if($t_id){
				//更新用户任务额
                $sql = "SELECT s_id FROM ". $GLOBALS['ecs']->table('user_stock') .
                        " WHERE status = 0 AND user_id = '$task[user_id]'";
                $res = $GLOBALS['db']->getRow($sql);
                if($res){
                    $sql = "UPDATE " . $GLOBALS['ecs']->table('user_stock') .
                        " SET now_assess_value = now_assess_value + ('$task[task_value]') " .
            ...
            if($user_order_info['parent_id'] != 0){
                $up_user_rankid = get_user_rankid($user_order_info['parent_id']);
...
function get_goods_thumb($goods_id){
	if($goods_id){
    	...
    	$result_goods_thumb = $GLOBALS['db']->getOne($sql);
		return $result_goods_thumb;
	}
}
...
function logg($b){
	...
	$sql = "INSERT INTO " . $GLOBALS['ecs']->table('zrizhi') .
            " (qq, ip,time)".
            " VALUES ('$ky','$ip','$time')";
    $GLOBALS['db']->query($sql);
}
//获取会员等级所属ID
function get_user_rankid($user_id){
	/* 更新会员等级 */
	$sql = "SELECT `user_id` , `rank_points` , `user_rank`".
	...
	$sql2 = "SELECT `rank_id` , `min_points` , `max_points` , `special_rank`".
...
function get_user_rank_info($user_id,$type=''){
	/* 获取会员贡献点数 */
	$sql = "SELECT `user_id` , `rank_points` , `user_rank`".
           " FROM " .$GLOBALS['ecs']->table('users'). "".
           " WHERE `user_id` = '" . $user_id . "'";
...
function change_shop_value($code,$value,$type){
	if($type == 1 || $type == 3){
		$sql = 'SELECT value FROM ' . $GLOBALS['ecs']->table('shop_config') .
            " WHERE code = '$code'";
		$old_value = $GLOBALS['db']->GetOne($sql);
		if($type == 1){
	...
	$sql = "UPDATE ".$GLOBALS['ecs']->table('shop_config'). " SET value = '$new_value' WHERE code = '$code'";
    $GLOBALS['db']->query($sql);
}

//获取系统设置的某个字段的值
function get_shop_config_value($code){
	$sql = "SELECT `value`".
...
//获取用户的真实姓名
function get_user_tname($user_name){
	$sql = "SELECT `user_id` , `tname`".
           " FROM " .$GLOBALS['ecs']->table('users'). "".
           " WHERE `user_name` = '" . $user_name . "'";	   
    return $GLOBALS['db']->getRow($sql);
}
//获取用户的头像或者昵称(type=1时返回头像，2返回昵称，3返回头像和昵称)2016-3-13	
function get_user_nc_tx($uid,$type = ''){
	$sql = "SELECT `u_headimg` , `nicheng` , `user_name`".
			" FROM " .$GLOBALS['ecs']->table('users'). "".
			" WHERE `user_id` = '" . $uid . "'";
	$res = $GLOBALS['db']->getRow($sql);
	$sql = "SELECT `headimgurl` , `nickname`".
			" FROM " . $GLOBALS['ecs']->table('wechat_user'). "".
			" WHERE `ect_uid` = '" . $uid . "'";
	$res2 = $GLOBALS['db']->getRow($sql);
	if($type==1){
		if($res['u_headimg']){
...
//更新所有用户的股数及比例
function change_user_stock($step = ''){
    if($step == '1'){
		$change_desc = "结算前更新";
		$change_type = 1;
	}elseif($step == '2'){
    ...
    $sum = get_all_user_stock();
    if(abs(intval(floatval($all_premium)+floatval($pure_profit)) - intval($sum['all_stock'])) > 10){
        //获取用户当前的股份数
        $sql = "SELECT s_id,user_id,stock FROM " . $GLOBALS['ecs']->table('user_stock') . " WHERE stock > 0";
        ...
        foreach($rs as $k=>$v){
			...
			if($change_stock <> '0'){
				//更新用户股数及比例
				log_stock($v['s_id'],$v['user_id'],$change_stock,$proportion,'',$change_desc,$change_type);
			}	
        }
    }           
}
//获取所有用户的股数及合伙人数量
function get_all_user_stock(){
    $sql = "SELECT SUM(stock) as all_stock,COUNT(user_id) as user_number FROM " . $GLOBALS['ecs']->table('user_stock') . " WHERE stock > 0";
    return $GLOBALS['db']->getRow($sql);
}

//修改股数并添加流水里
function log_stock($s_id,$user_id,$stock = 0,$proportion = '',$exp = '',$change_desc = '', $change_type = ''){
    /* 插入帐户变动记录 */
    $account_log = array(
        's_id' => $s_id,
        'user_id' => $user_id,
    ...
    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('stock_log'), $account_log, 'INSERT');
...
//执行分红
function user_dividend_begin($fh_value){
    $sum = get_all_user_stock();
	//logg($sum);
    //获取用户当前的股份数
    $sql = "SELECT s_id,user_id,stock FROM " . $GLOBALS['ecs']->table('user_stock') . " WHERE stock > 0";
    ...
    foreach($rs as $k=>$v){
        ...
        $change_desc = "平台分红";
		if($user_fh <> '0'){
			log_account_change($v['user_id'],$user_fh,0,0,0,$change_desc,30,2);
		}        
    }
    return $sj_fh;
}
...
function log_profit($value,$change_type,$change_desc=''){
    ...
    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('profit_log'), $data, 'INSERT');
}
//插入发放红包的结果
function log_hongbao($wp_result,$user_id){
	$time = date('Y-m-d H:i:s');
	if($wp_result['result_code']=='SUCCESS'){		
		$sql = "INSERT INTO " . $GLOBALS['ecs']->table('wpay_hongbao_log') .
				" (user_id, return_code, return_msg, result_code, mch_billno, re_openid, total_amount, send_listid, send_time,action_time)".
				" VALUES ('$user_id','$wp_result[return_code]','$wp_result[return_msg]','$wp_result[result_code]','$wp_result[mch_billno]','$wp_result[re_openid]','$wp_result[total_amount]','$wp_result[send_listid]','$wp_result[send_time]','$time')";	
	}else{
		$sql = "INSERT INTO " . $GLOBALS['ecs']->table('wpay_hongbao_log') .
				" (user_id, return_code, return_msg, result_code, mch_billno, re_openid, total_amount, err_code_des,action_time)".
				" VALUES ('$user_id','$wp_result[return_code]','$wp_result[return_msg]','$wp_result[result_code]','$wp_result[mch_billno]','$wp_result[re_openid]','$wp_result[total_amount]','$wp_result[err_code_des]','$time')";
	}
	$GLOBALS['db']->query($sql);
}

//获取利润情况
function get_op_info($order_id){
	$sql = "SELECT * FROM " . $GLOBALS['ecs']->table('ab_log') . " WHERE order_id = $order_id";
    return $GLOBALS['db']->getAll($sql);
}	
