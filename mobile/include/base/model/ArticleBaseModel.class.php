...
class ArticleBaseModel extends BaseModel {
    ...
    public function get_cat_articles($cat_id, $page = 1, $size = 20, $requirement = '') {
        ...
        $list = $this->select($condition, 'article_id, title, author,description, add_time, file_url, open_type', ' article_id DESC', $limit);
        ...
        if (is_array($list)) {
            foreach ($list as $vo) {
				...
				$arr[$article_id]['description'] = sub_str($vo['description'], 90);
