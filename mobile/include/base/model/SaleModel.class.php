...
class SaleModel extends BaseModel {
                ...
                if ($up_uid) {
                    if (!empty($affiliate['config']['level_register_all'])) {
                        if (!empty($affiliate['config']['level_register_up'])) {
                            ...
                            model('ClipsBase')->log_account_change($up_uid, 0, 0, $affiliate['config']['level_register_all'], $affiliate['config']['pay_register_all'], L('register_affiliate2'));
                        }
                    }
    ...
    public function get_line_list($num, $start){
        ...
        foreach ($res as $k => $v) {
			...
			$res[$k]['nickname']=$this->get_nickname($v['user_id']);
        }
     ...
     * @param int $start 每页开始显示条数
     */
	public function get_all_line_list($num, $start,$layer){

        //$sql = 'SELECT * FROM ' . $this->pre . "users WHERE parent_id = " . session('user_id') . " ORDER BY user_id DESC limit " . $start . ',' . $num;
        $res = $this->get_array(session('user_id'));
        
        return $res;
    }
	
	public function get_array($id=0){ 
		$sql = "select user_id,user_name from " . $this->pre . "users where parent_id= ".$id; 
		$result = $this->query($sql);//查询子类
		//return $result; 
		return mysql_fetch_assoc($result);
		$arr = array(); 
		if($result && mysql_affected_rows()){//如果有子类 
			while($rows = mysql_fetch_assoc($result)){ //循环记录集 
				$rows['list'] = get_array($rows['user_id']); //调用函数，传入参数，继续查询下级 
				$arr[] = $rows; //组合数组 
			} 
			return $arr; 
		} 
	...
	*/
	public function get_nickname($uid){
		$result=$this->model->table('wechat_user')->field('nickname')->where('ect_uid='.$uid)->getOne();
		return $result;
	}
    /**
     * 获取分销商id
    ...
    function get_user_orders($parent_id, $pay = 1, $num = 10, $start = 0 ,$user_id) {
        ...
        $sql = "SELECT order_id, order_sn, shipping_id, user_id, order_status, shipping_status, pay_status, add_time, is_separate, " .
        ...
        foreach ($res as $key => $value) {
			...
			$k_order_status=$value['order_status'];
            ...
            $arr[] = array('order_id' => $value['order_id'],
				...
				'nickname' => model('ClipsBase')->get_touxiang($value['user_id'],2),
				...
				'is_pay' => model('ClipsBase')->get_order_status($k_order_status),
				'goods'=>$goods_list,
                );	
        }
        return $arr;
     ...
     * @return  array       $order_list     订单列表
     */
    function get_user_orders2($pay = 1, $num = 10, $start = 0 ,$user_id) {
        /* 取得订单列表 */
        $arr = array();
        ...
        foreach ($res as $key => $value) {
            ...
            $arr[] = array('order_id' => $value['order_id'],
                'order_sn' => $value['order_sn'],
				'user_id' => $value['user_id'],
				'nickname' => model('ClipsBase')->get_user_nc_tx($value['user_id'],2),
                'img' => get_image_path(0, model('Order')->get_order_thumb($value['order_id'])),
                'order_time' => local_date(C('time_format'), $value['add_time']),
                'order_status' => $value['order_status'],
                'shipping_id' => $value['shipping_id'],
    ...
    public function get_sale_goods_count(){
        $sql =  "select order_id from " . M()->pre . "order_info where parent_id=".session('user_id');
        $arr_order_id = M()->query($sql);
        $order_id = '';
        if(!empty($arr_order_id)){
            ...
            $sql = "select sum(goods_number) as count from ".M()->pre."order_goods where order_id in (".$order_id.")";
            $res = M()->query($sql);
	...
	public function gsh_down($arr){
		if($arr[1]['uid']!=''){
			$down1 = explode(',',$arr[1]['uid']);
			foreach($down1 as $k=>$v){
				$down1[$k]= $this->get_ziliao($v,1);
			}
		}else{
			$down1=array();
		}
		if($arr[2]['uid']!=''){
			$down2 = explode(',',$arr[2]['uid']);
			foreach($down2 as $k=>$v){
				$down2[$k]= $this->get_ziliao($v,2);
			}
		}else{
			$down2=array();
		}
		if($arr[3]['uid']!=''){
			$down3 = explode(',',$arr[3]['uid']);
			foreach($down3 as $k=>$v){
				$down3[$k]= $this->get_ziliao($v,3);
			}
		}else{
			$down3=array();
		}	
		$down = array_merge($down1,$down2,$down3);
		return $down;
	}
	
	public function get_ziliao($uid,$ceng){
		$sql =  "select user_id,reg_time,nicheng,user_name,mobile_phone from " . $this->pre . "users where user_id=".$uid;
		$result = $this->row($sql);
		$result['reg_time']    =   local_date(C('date_format'), $result['reg_time']);
        $result['mobile_phone']    =    $result['mobile_phone'] ? substr_replace($result['mobile_phone'],'****',3,4) : '';
		if(empty($result['nicheng'])){
			$result['nicheng'] = model('ClipsBase')->get_user_nc_tx($uid,2);			
		}
		$result['rank_name'] = model('ClipsBase')->get_user_rank($uid,2);
		if($ceng==1){
			$result['ceng']="嫡系部落";
		}elseif($ceng==2){
			$result['ceng']="旁系部落";
