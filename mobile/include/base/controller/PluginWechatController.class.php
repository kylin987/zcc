...
abstract class PluginWechatController
{
     ...
     * 两个积分赠送处理
     */
    public function do_point2($fromusername, $info, $point_value, $point_value2)
    {
        $time = time();
        $user_id = model('Base')->model->table('wechat_user')
            ->field('ect_uid')
            ->where('openid = "' . $fromusername . '"')
            ->getOne();
		$user_info = model('Base')->model->table('users')
			->field('rank_points,pay_points')
			->where('user_id = ' . $user_id)
			->find();			
        // 增加积分
        $point['rank_points'] = intval($user_info['rank_points']) + intval($point_value);
		$point['pay_points'] = intval($user_info['pay_points']) + intval($point_value2);
        model('Base')->model->table('users')
            ->data($point)
            ->where('user_id = ' . $user_id)
            ->update();
        ...
        $data['frozen_money'] = 0;
        $data['rank_points'] = $point_value;
		$data['pay_points'] = $point_value2;        
        $data['change_time'] = $time;
        $data['change_desc'] = $info['name'] . '银币、积分赠送';
        $data['change_type'] = ACT_OTHER;
     ...
     * 银币消耗与赠送
     *
     * @param 用户的openid $fromusername            
     * @param 操作银币数 $num   
	          
     */
    public function edit_yinbi($fromusername,$num,$plug_name)
    {
        $time = time();
        $user_id = model('Base')->model->table('wechat_user')
            ->field('ect_uid')
		...
		$point['pay_points'] = intval($user_info['pay_points']) + intval($num);
	...
	//获取用户的银币数
	public function get_yinbi($openid){
		$user_id = model('Base')->model->table('wechat_user')
			->field('ect_uid')
			->where('openid = "' . $openid . '"')
			->getOne();
		$user_yinbi = model('Base')->model->table('users')
			->field('pay_points')
			->where('user_id = "' . $user_id . '"')
			->getOne();
		if($user_yinbi){
	...
	public function get_address($openid){
		$user_id = model('Base')->model->table('wechat_user')
			...
			->where('openid = "' . $openid . '"')
			->getOne();
		$user_address = model('Base')->model->table('user_address')
			->field('address_id,user_id,consignee,country,province,city,district,address,mobile')
			->where('user_id = "' . $user_id . '"')
		...
		if($user_address){
			foreach($user_address as $k=>$v){
				if($k=="province"){
					$address .= model('RegionBase')->get_region_name($v);
					$address .="省";
				}
				if($k=="city"){
					$address .= model('RegionBase')->get_region_name($v);
			...
			$result['uname']=$user_address['consignee'];
			$result['phone']=$user_address['mobile'];
			return $result;
		}else{
			return 0;
		}	
	...
	public function rand_str(){
		$arr=array('差一点就中了','再换下手试试哦','要再努力一下啦','没有中奖，再来一次','很遗憾，没有中哦','再接再厉');
		$v=intval(mt_rand(0,5));
		return $arr[$v];
	}
