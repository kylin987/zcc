...
class FlowController extends CommonController {
    ...
    public function checkout() {
		...
		/* 取得用户金币 */
        //$use_surplus = C('use_surplus');
        if ($_SESSION['user_id'] > 0 && $user_info['gold_coin'] > 0) {
            // 能使用金币
            $this->assign('allow_use_gold_coin', 1);
            $this->assign('your_gold_coin', $user_info ['gold_coin']);
        }

        /* 如果使用积分，取得用户可用积分及本订单最多可以使用的积分 */
        $use_integral = C('use_integral');
        if ((!isset($use_integral) || C('use_integral') == '1') && $_SESSION ['user_id'] > 0 && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS)) {
            // 能使用积分
            $this->assign('allow_use_integral', 1);
            $this->assign('order_max_integral', model('Flow')->flow_available_points()); // 可用积分
			$this->assign('order_min_integral', model('Flow')->flow_available_points2()); // 必用积分
		...
		/* 判断是否是未消费用户，如果是，则允许填写推荐码，以修改推荐人 */
		//$user_rankid = model('ClipsBase')->get_user_rank($_SESSION ['user_id']);
		$user_rankpoints = model('ClipsBase')->get_user_jifen2($_SESSION ['user_id']);
		if($user_rankpoints==0){
			$this->assign('upid_show', 1);
		}
    ...
    public function done() {
		...
		//检查接收的推荐码
		if($_POST ['up_id']){
			$up_id=decodeId($_POST ['up_id']);
			$show_info=model('ClipsBase')->check_upid($up_id,$_SESSION ['user_id']);
			if($show_info){
				show_message($show_info, '', '', 'warning');
			}
		}
        // 订单信息
        $order = array(
			...
			'up_id' => isset($up_id) ? $up_id : 0,
            'card_message' => trim($_POST ['card_message']),
            'surplus' => isset($_POST ['surplus']) ? floatval($_POST ['surplus']) : 0.00,
            'integral' => isset($_POST ['integral']) ? intval($_POST ['integral']) : 0,
			'goldcoin' => isset($_POST ['goldcoin']) ? floatval($_POST ['goldcoin']) : 0,
        ...
        if ($user_id > 0) {
			...
			//查询用户有多少金币
			$user_glodcoin = $user_info['gold_coin'];
			if($order ['goldcoin'] > floatval($user_glodcoin)){
				$order ['goldcoin'] = 0;
			}
        ...
        if ($order ['user_id'] > 0 && $order ['surplus'] > 0) {
            $account_log = model('ClipsBase')->log_account_change3($order ['user_id'], $order ['surplus'] * (- 1), 0, 0, 0, sprintf(L('pay_order'), $order ['order_sn']),99,2);
			//更新到订单详情里
		...
		if ($order ['user_id'] > 0 && $order ['goldcoin'] > 0) {
            model('ClipsBase')->log_account_change2($order ['user_id'], 0, 0,$order ['goldcoin'] * (- 1), 0, 0, sprintf(L('pay_order'), $order ['order_sn']));
        }
		...
		$tz_info="您的粉丝<".$chenghu.">提交了订单".$order['order_sn']."。您可以到分销订单中查看";
		model('ClipsBase')->send_wechat_msg(2,$order['user_id'],$tz_info,0);
		*/
        /* 如果订单金额为0 处理虚拟卡 */
        if ($order ['order_amount'] <= 0) {
		...
		//获取上级用户昵称并插入到order数组
        $order['nicheng']=model('ClipsBase')->get_user_nc_tx($order['up_id'],2);
    ...
    public function change_integral() {
        ...
        $flow_points = model('Flow')->flow_available_points();  // 该订单允许使用的积分
		$flow_min_points = model('Flow')->flow_available_points2();  // 该订单必须使用的积分
        $user_points = $user_info['pay_points']; // 用户的积分总数
        ...
        if ($points > $user_points) {
			...
			$result['vau'] = floatval($flow_points);
        } elseif ($points < $flow_min_points) {
            $result['error'] = sprintf(L('integral_too_less'), $flow_min_points);
			$result['vau'] = floatval($flow_min_points);
        } else {
            /* 取得购物类型 */
            $flow_type = isset($_SESSION['flow_type']) ? intval($_SESSION['flow_type']) : CART_GENERAL_GOODS;
     ...
     * 改变金币
     */
    public function change_goldcoin() {
        $goldcoin = floatval($_GET['goldcoin']);		
        ...
        //$flow_points = model('Flow')->flow_available_points();  // 该订单允许使用的积分
        $user_goldcoin = $user_info['gold_coin']; // 用户的金币总数

        if ($goldcoin > $user_goldcoin) {
            $result['error'] = L('goldcoin_not_enough');
        } else {
            /* 取得购物类型 */
            $flow_type = isset($_SESSION['flow_type']) ? intval($_SESSION['flow_type']) : CART_GENERAL_GOODS;

            $order['goldcoin'] = $goldcoin;
    ...
    public function check_integral() {
		...
		if ($points < $flow_min_points) {
            die(sprintf(L('integral_too_less'), $flow_min_points));
        }
