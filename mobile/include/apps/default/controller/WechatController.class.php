    ...
    private function subscribe($openid = '', $scene_id = 0)
    {
        if(!empty($openid)){
            ...
            if (empty($rs)) {
                ...
                if(empty($ect_uid)){
					//判断用户是否通过微信扫描登陆过PC端
					$aite_wx_id="wx_".$info['unionid'];
					$resu_wx_user = model('Base')->model->table('users')
						->field('user_id,user_name')
						->where('aite_wx_id = "' . $aite_wx_id . '"')
						->find();
					if($resu_wx_user){
						//如果有值，说明之前用微信扫描登陆过PC端                	
						$template = '您已拥有帐号，用户名为'.$resu_wx_user['user_name'];
						$data['ect_uid'] = $resu_wx_user['user_id'];
					}else{
						// 设置的用户注册信息
				...
				//推送文字信息
				/*
				$tem = '欢迎光临千康元微销营销平台。'."\r\n".'<a href = "http://www.qiankangyuan.com/mobile/index.php?c=topic&a=index&id=1">全国招商简章>></a>'."\r\n".'<a href = "http://www.qiankangyuan.com/mobile/index.php?m=default&c=article&a=index">玛咖保健文库入口>></a>'."\r\n".'<a href = "http://www.qiankangyuan.com/mobile/index.php?m=default&c=goods&a=index&id=45">黑玛咖精粉含片购买>></a>'."\r\n".'<a href = "http://www.qiankangyuan.com/mobile/index.php?m=default&c=goods&a=index&id=47">黑玛咖--干果购买>></a>';
				$msg = array(
					'touser' => $openid,
					'msgtype' => 'text',
					'text' => array(
    ...
    static function update_weixin_user($userinfo, $wechat_id, $weObj)
    {
        ...
        if (empty($ret)) {
            ...
            if(empty($ect_uid)){
				...
				//判断用户是否通过微信扫描登陆过PC端
				$aite_wx_id="wx_".$userinfo['unionid'];
				$resu_wx_user = model('Base')->model->table('users')
					->field('user_id,user_name')
					->where('aite_wx_id = "' . $aite_wx_id . '"')
					->find();
				if($resu_wx_user){
					//如果有值，说明之前用微信扫描登陆过PC端                	
                	$template = '您已拥有帐号，用户名为'.$resu_wx_user['user_name'];
                	$data1['ect_uid'] = $resu_wx_user['user_id'];
				}else{
					...
					if (model('Users')->register($username, $password, $username . '@' . $domain) !== false) {
						$data['user_rank'] = 0;					
						if($_COOKIE['sale_id']){						
							$data['parent_id'] = $_COOKIE['sale_id'];																	
						}					
