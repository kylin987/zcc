...
class PaymentModel extends BaseModel {
	...
	function check_pay_log($log_id){
		
		$sql = 'SELECT o.pay_status' .
                ' FROM ' . $this->pre . 'order_info AS o ' .
                ' LEFT JOIN ' . $this->pre . 'pay_log AS p' .
	...
	function log_get_orderinfo($log_id){
		
		$sql = 'SELECT o.pay_status,o.order_id,o.order_sn,o.user_id,o.up_id,u.rank_points' .
                ' FROM ' . $this->pre . 'order_info AS o ' .
                ' LEFT JOIN ' . $this->pre . 'pay_log AS p' .				
                ' ON p.order_id = o.order_id ' .
	...
	function log_get_chongzhi_info($log_id){		
		$sql = 'SELECT u.is_paid,u.id' .
                ' FROM ' . $this->pre . 'user_account AS u ' .
                ' LEFT JOIN ' . $this->pre . 'pay_log AS p' .
                ' ON p.order_id = u.id ' .
                " WHERE p.log_id = '$log_id'";
        $res = $this->row($sql);
	...
	function log_get_daili_info($log_id){		
		$sql = 'SELECT u.is_paid,u.id' .
                ' FROM ' . $this->pre . 'user_account AS u ' .
                ' LEFT JOIN ' . $this->pre . 'pay_log AS p' .
                ' ON p.order_id = u.id ' .
            ...
            if ($pay_log && $pay_log['is_paid'] == 0) {
                ...
                if ($pay_log['order_type'] == PAY_ORDER) {
                    ...
                    if (empty($res_id)) {
                        ...
                        model('ClipsBase')->log_account_change($arr['user_id'], $arr['amount'], 0, 0, 0, "充值", ACT_SAVING);
                    }
                }
            } else {
                /* 取得已发货的虚拟商品信息 */
