...
class OrderModel extends BaseModel {
    ...
    function user_info($user_id) {
        ...
        if ($user) {
			...
			$user['formated_goldcoin'] = price_format($user['gold_coin'], false);
        }
        return $user;
    }
    ...
    function last_shipping_and_payment() {
        ...
        if (empty($row)) {
            /* 如果获得是一个空数组，则返回默认值 */
            $row = array('shipping_id' => 20, 'pay_id' => 4);
        }
    ...
    function flow_order_info() {		
		...
		//如果配送方式和支付方式为0，那么修改为默认的
		if($order['shipping_id'] == 0){
			$order['shipping_id'] = 20;
		}
		if($order['pay_id'] == 0){
			$order['pay_id'] = 4;
		}
