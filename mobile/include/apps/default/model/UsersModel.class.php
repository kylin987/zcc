...
class UsersModel extends BaseModel {
    ...
    function register($username, $password, $email, $other = array()) {	
        ...
        if (!ECTouch::user()->add_user($username, $password, $email)) {
            ...
            //定义other合法的变量数组
            $other_key_array = array('msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone', 'tname', 'parent_id');
                   ...
                   if (!empty($affiliate['config']['level_register_all'])) {
                        if (!empty($affiliate['config']['level_register_up'])) {
                            ...
                            if ($res['rank_points'] + $affiliate['config']['level_register_all'] <= $affiliate['config']['level_register_up']) {							
                                model('ClipsBase')->log_account_change($up_uid, 0, 0, $affiliate['config']['level_register_all'], $affiliate['config']['pay_register_all'], sprintf(L('register_affiliate2'), $_SESSION['user_id'], $username));
								$tongz="您已获赠:积分".$affiliate['config']['level_register_all']."分,银币".$affiliate['config']['pay_register_all']."枚";
                            }else{
								model('ClipsBase')->log_account_change($up_uid, 0, 0, 0, $affiliate['config']['pay_register_all'], sprintf(L('register_affiliate2'), $_SESSION['user_id'], $username));
								$tongz="您已获赠:银币".$affiliate['config']['pay_register_all']."枚";
							}
                        } else {						
                           model('ClipsBase')->log_account_change($up_uid, 0, 0, $affiliate['config']['level_register_all'], $affiliate['config']['pay_register_all'], sprintf(L('register_affiliate2'), $_SESSION['user_id'], $username));
							$tongz="您已获赠:积分".$affiliate['config']['level_register_all']."分,银币".$affiliate['config']['pay_register_all']."枚";
					...
					$chenghu=getChenghu($_SESSION['user_id']);
                    $this->query($sql);
					//微信提醒上级
					/*					
					$template2 = "您有新增粉丝<".$chenghu . ">\r\n" . $tongz;
					//model('Users')->xierizhi("下面是上级ID");
					//model('Users')->xierizhi($up_uid);
					//model('Users')->xierizhi($template2);
					model('ClipsBase')->send_wechat_msg(1,$up_uid,$template2,0);
					*/
                }
            }
    ...
    function edit_profile($profile) {
        ...
        /* 过滤非法的键值 */
        $other_key_array = array('msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone','nicheng','tname','raply_bank','raply_kaihu','raply_username','raply_number','raply_beizhu');
    ...
    function get_profile($user_id) {
        ...
        $info = array();
        $infos = array();
        $sql = "SELECT user_name, birthday, sex, question, answer,nicheng,u_headimg, rank_points, pay_points,user_money, user_rank," .
                " msn,tname, qq, office_phone, home_phone, mobile_phone, passwd_question, passwd_answer,raply_bank,raply_kaihu,raply_username,raply_number,raply_beizhu " .
        ...
        $info['qq'] = $infos['qq'];
        $info['msn'] = $infos['msn'];
		$info['tname'] = $infos['tname'];
		$info['nicheng'] = $infos['nicheng'];
		$info['u_headimg'] = $infos['u_headimg'];
		...
		$info['raply_bank'] = $infos['raply_bank'];
        $info['raply_kaihu'] = $infos['raply_kaihu'];
		$info['raply_username'] = $infos['raply_username'];
        $info['raply_number'] = $infos['raply_number'];
        $info['raply_beizhu'] = $infos['raply_beizhu'];
    ...
    function cancel_order($order_id, $user_id = 0) {
        /* 查询订单信息，检查状态 */
        $sql = "SELECT user_id, order_id, order_sn , surplus , integral , goldcoin , bonus_id, order_status, shipping_status, pay_status FROM " . $this->pre . "order_info WHERE order_id = '$order_id'";
        $order = $this->row($sql);
        ...
        if ($this->query($sql)) {
            ...
            if ($order['user_id'] > 0 && $order['surplus'] > 0) {
                ...
                model('ClipsBase')->log_account_change9($order['user_id'], $order['surplus'], 0, 0, 0, $change_desc,$order['order_sn']);
            }
			...
			if ($order['user_id'] > 0 && $order['goldcoin'] > 0) {
                $change_desc = sprintf(L('return_goldcoin_on_cancel'), $order['order_sn']);
                model('ClipsBase')->log_account_change2($order['user_id'], 0, 0, $order['goldcoin'], 0, 0, $change_desc);
            }
     ...
     * @param   int         $start           显示起始位置
     *
     * @return  array       $arr             大礼包列表
     */
    function get_user_spree_list($user_id, $num = 10, $start = 0) {
        $sql = "SELECT * FROM " . $this->pre . "user_spree " .
                " WHERE is_sell <> 2  AND is_del = 0 AND user_id = '" . $user_id . "' ORDER BY is_check DESC,ex_time DESC LIMIT $start , $num";
        $res = $this->query($sql); 
		foreach ($res as $k => $v) {
            $res[$k]['ex_time'] = local_date(C('date_format'), $v['ex_time']);
			if($res[$k]['check_time'] != 0){
				$res[$k]['check_time'] = local_date(C('date_format'), $v['check_time']);
			}else{
				$res[$k]['check_time'] = "";
			}
			if($res[$k]['is_check']==1){
            ...
            $res[$k]['is_check'] = $res[$k]['is_check']==1 ? "<font color=blue>已激活</font>":"<font color=red>未激活</font>";
     ...
     * @param   int         $start           显示起始位置
     *
     * @return  array       $arr             提交购买大礼包列表
     */
    function get_user_buy_spree_list($user_id, $num = 10, $start = 0) {
        $sql = "SELECT * FROM " . $this->pre . "spree_log " .
                " WHERE trade_type = 0 AND user_id = '" . $user_id . "' ORDER BY tr_id DESC LIMIT $start , $num";
        $res = $this->query($sql); 
		foreach ($res as $k => $v) {
            $res[$k]['send_time'] = local_date(C('date_format'), $v['send_time']);
			if($res[$k]['status'] == 0){
				$res[$k]['s_note'] = "未审核";
     ...
     * @param   int         $start           显示起始位置
     *
     * @return  array       $arr             出售大礼包列表
     */
    function get_user_sell_spree_list($user_id, $num = 10, $start = 0) {
        $sql = "SELECT * FROM " . $this->pre . "spree_log " .
                " WHERE trade_type = 1 AND user_id = '" . $user_id . "' ORDER BY tr_id DESC LIMIT $start , $num";
        $res = $this->query($sql); 
		foreach ($res as $k => $v) {
            $res[$k]['send_time'] = local_date(C('date_format'), $v['send_time']);
			if($res[$k]['status'] == 0){
    ...
    function order_fee($order, $goods, $consignee) {
		...
		/* 金币 */
		$order['goldcoin'] = $order['goldcoin'] > 0 ? $order['goldcoin'] : 0;
        if ($total['amount'] > 0) {
            if (isset($order['goldcoin']) && $order['goldcoin'] > $total['amount']) {
                $order['goldcoin'] = $total['amount'];
                $total['amount'] = 0;
            } else {
                $total['amount'] -= floatval($order['goldcoin']);
            }
        } else {
            $order['goldcoin'] = 0;
            $total['amount'] = 0;
        }
        $total['goldcoin'] = $order['goldcoin'];
        $total['goldcoin_formated'] = price_format($order['goldcoin'], false);
        ...
        elseif($_SESSION['user_id'] !== 0){			
            ...
            if (!empty($config['config']['expire'])) {
                ...
                if(gmtime() - $reg_info['reg_time'] <= $eff_time){
					if($reg_info['parent_id'] != 0){
						return $reg_info['parent_id'];
					}else{
                    	return $_COOKIE['sale_id'];
					}	
                }
            }
        }		
     ...
     * @return  array
     */
    public function get_account_detail2($user_id, $num, $start) {
        
        // 获取余额记录
        $account_log = array();
        
        $sql = 'SELECT * FROM ' . $this->pre . "account_log WHERE user_id = " . $user_id . ' AND gold_coin <> 0' .
        " ORDER BY log_id DESC limit " . $start . ',' . $num;
        $res = $this->query($sql);        
        ...
        foreach ($res as $k => $v) {
            $res[$k]['change_time'] = local_date(C('time_format'), $v['change_time']);
            $res[$k]['type'] = $v['gold_coin'] > 0 ? L('account_inc') : L('account_dec');
            $res[$k]['gold_coin'] = price_format(abs($v['gold_coin']), false);
            $res[$k]['frozen_gold'] = price_format(abs($v['frozen_gold']), false);
            $res[$k]['rank_points'] = abs($v['rank_points']);
            $res[$k]['pay_points'] = abs($v['pay_points']);
            $res[$k]['short_change_desc'] = sub_str($v['change_desc'], 60);
            $res[$k]['amount'] = $v['gold_coin'];
        }
     ...
     * @return  array
     */
    public function get_account_detail3($user_id, $num, $start) {
        
        // 获取余额记录
        $account_log = array();
        
        $sql = 'SELECT * FROM ' . $this->pre . "account_log WHERE user_id = " . $user_id . ' AND pay_points <> 0' .
        " ORDER BY log_id DESC limit " . $start . ',' . $num;
        $res = $this->query($sql);        
     ...
     * @return  array
     */
    public function get_account_detail4($user_id, $num, $start) {
        
        // 获取余额记录
        $account_log = array();
        
        $sql = 'SELECT * FROM ' . $this->pre . "account_log WHERE user_id = " . $user_id . ' AND rank_points <> 0' .
        " ORDER BY log_id DESC limit " . $start . ',' . $num;
        $res = $this->query($sql);        
        ...
        foreach ($res as $k => $v) {
            $res[$k]['change_time'] = local_date(C('date_format'), $v['change_time']);
            $res[$k]['type'] = $v['rank_points'] > 0 ? L('account_inc') : L('account_dec');
            $res[$k]['gold_coin'] = price_format(abs($v['gold_coin']), false);
            $res[$k]['frozen_gold'] = price_format(abs($v['frozen_gold']), false);
            $res[$k]['rank_points'] = abs($v['rank_points']);
            $res[$k]['pay_points'] = abs($v['pay_points']);
            $res[$k]['short_change_desc'] = sub_str($v['change_desc'], 60);
            $res[$k]['amount'] = $v['rank_points'];
        }
	...
	public function getOpenid($u_id){
		$openid=$this->model->table('wechat_user')
			->field('openid')
			->where('ect_uid = "' .$u_id . '"')
			->find();
		if (empty($openid)) {
	...
	public function xierizhi($b){
		...
		$sql = 'INSERT INTO ' . $this->pre . 'zrizhi (qq,ip,time) ' .
                " VALUES ('$ky','$ip','$time')";
        $this->query($sql); 
	}
}
