...
class FlowModel extends BaseModel {
    ...
    function flow_available_points2() {
        $sql = "SELECT SUM(g.integral * c.goods_number) as sum " . "FROM " . $this->pre . "cart AS c, " . $this->pre . "goods AS g " . "WHERE c.session_id = '" . SESS_ID . "' AND c.goods_id = g.goods_id AND c.is_gift = 0 AND g.integral > 0 " . "AND g.is_static = 1 AND c.rec_type = '" . CART_GENERAL_GOODS . "'";
