...
/**
     ...
     * @return array
     */
    function k_get_config($orgid){		
        $config = M()->table('wechat')
	...
	//获取opendi
	function getOpenid($u_id){
		$openid= M()->table('wechat_user')
			->field('openid')
			->where('ect_uid = "' .$u_id . '"')
			->find();
		if (empty($openid)) {
	...
	function getChenghu($u_id){
		$nickname= M()->table('wechat_user')
			->field('nickname')
			->where('ect_uid = "' .$u_id . '"')
			->find();
		if (empty($nickname)) {
            $username= M()->table('users')
				->field('user_name')
				->where('user_id = "' .$u_id . '"')
				->find();
	...
	//获取上级ID
	function getUpid($u_id){
		$parent_id= M()->table('users')
			->field('parent_id')
			->where('user_id = "' .$u_id . '"')
			->find();
	...
	//根据订单号获取上级ID
	function getUpid2($order_sn){
		$parent_id= M()->table('order_info')
			->field('parent_id')
	...
	function encodeId($uid){
		...
		return $rands.$arr[$length2].$uid;
	}
	//对id进行简单解密
	function decodeId($str){
		$b=preg_replace('/^0*/', '', preg_replace('/\D/s', '', $str));
		//$c=preg_replace('/^0*/', '', $b);
    ...
    //金额简单化，去掉角和分
    function p_easy($value){
        return floor($value)."元";
    }
	...
	function logg($b){
		...
		$sql = 'INSERT INTO ' . M()->pre . 'zrizhi (qq,ip,time) ' .
                " VALUES ('$ky','$ip','$time')";
        M()->query($sql); 
	}
