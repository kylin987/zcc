...
class Image {
    ...
    static public function buildImageVerify($width = 48, $height = 22, $randval = NULL, $verifyName = 'ectouch_verify') {
        ...
        shuffle($arr);
        $randval = empty($randval) ? ("" . substr(implode('', $arr), 4, 4)) : $randval;
     ...
     * @$$waterPos 水印位置(0-9) 0为随机，其他代表上中下9个部分位置
     */
    static public function water2($image, $water, $waterPos = 9,$ximage) {
        //检查图片是否存在
        if (!file_exists($image) || !file_exists($water))
            return false;
        //读取原图像文件
     ...
     * @$$waterPos 水印位置(0-9) 0为随机，其他代表上中下9个部分位置
     */
    static public function water3($text, $font, $size, $color, $x, $y,$image) {
        //检查图片是否存在
        if (!file_exists($image) || !file_exists($font))
            return false;        
		...
		#设置水印字体颜色
		$color = imagecolorallocate($image_im,255,255,255);		
		
		#打水印
		imagettftext($image_im,$size,0,$x,$y,$color,$font,$text);
		$bulitImg = "image" . $imageInfo['type'];
        $bulitImg($image_im, $image);
     ...
     * @$$waterPos 水印位置(0-9) 0为随机，其他代表上中下9个部分位置
     */
    static public function water4($text, $font, $size, $r,$g,$b, $x, $y,$im) {
        //检查图片是否存在
        if (!file_exists($font))
            return false;    
        
		
		$fontarea = imagettfbbox($size,0,$font,$text);
		$font_width = $fontarea[2]-$fontarea[0];
     ...
     * @$$waterPos 水印位置(0-9) 0为随机，其他代表上中下9个部分位置
     */
    static public function water5($text, $font, $size, $r,$g,$b, $x, $y,$im) {
        //检查图片是否存在
        if (!file_exists($font))
            return false;    
