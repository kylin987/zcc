        ...
        if (IS_GET && IS_AJAX) {
            ...
            if ($num > $config['free_prize_num']) {	                
				if($arg1){
					...
					if($u_yinbi<0){
						...
						$res = $this->edit_yinbi($openid,$xh_yinbi,"砸金蛋抽奖消费");
					}
							
				}else{
					$rs['status'] = 3;
                	$rs['msg'] = '免费抽奖次数已用完，是否消耗'.$config['xh_yinbi'].'银币继续砸金蛋';	
					echo json_encode($rs);
                	exit();				
				}
            ...
            if (! empty($prize)) {
                ...
                if($level == 'not'){
					...
					if($prize_type[$level]==1){
						$rs['msg'] = "恭喜您中了".$level."，获得".$prize_name[$level]."，奖品已发放至您的账户";
						$rs['status'] = 4;
						$rs['level'] = $level;
						$yb_msg="砸金蛋中了".$level."，获得".$prize_name[$level];
						$jl_yinbi = intval($prize_name[$level]);
						$yb_res = $this->edit_yinbi($openid,$jl_yinbi,$yb_msg);
					}else{
						$rs['msg'] = "恭喜您中了".$level."，获得".$prize_name[$level];
						$rs['status'] = 1;
						$rs['level'] = $level;
					}
                }
