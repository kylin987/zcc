        ...
        if (IS_GET && IS_AJAX) {
           ...
           $openid = session('openid');
					...
					if ($num > $config['free_prize_num']) {	                
						if($arg1){
							...
							if($u_yinbi<0){
								...
								$res = $this->edit_yinbi($openid,$xh_yinbi,"刮刮卡抽奖消费");
							}
            ...
            elseif($act == 'do'){
				$fre=$_GET['yb'];
				$type = $_GET['type'];
                $prize_type2 = I('get.prize_type') ? I('get.prize_type') : 0;
                $prize_name2 = I('get.prize_name') ? I('get.prize_name') : '';
                $prize_level2 = I('get.prize_level') ? I('get.prize_level') : 'not';
				if($prize_level2=="not"){
					$rs['msg'] = $prize_name[$prize_level2];
					$rs['status'] = 0;
					//$rs['level'] = $prize_level2;
				}else{					
					if($type==1){		//这里是判断银币还是实物
						$rs['msg'] = $prize_name2;
						$rs['status'] = 4;
						$rs['level'] = $prize_level2;
						$yb_msg="刮刮卡中了".$prize_level2."，获得".$prize_name2;
						$jl_yinbi = intval($prize_name2);
                ...
                // 抽奖记录
                $data['prize_type'] = $prize_type2;
                $data['wechat_id'] = $wxid;
                $data['openid'] = $openid;
                $data['prize_name'] = $prize_name2;
				if($yb_res){
					$data['issue_status'] = 1;
				}
				if($fre){
