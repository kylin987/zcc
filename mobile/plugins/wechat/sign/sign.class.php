...
plugin_name = $name;
    ...
    public function show($fromusername, $info)
    {
        ...
        if (isset($config['point_status']) && $config['point_status'] == 1) {
            ...
            if ($num < $config['point_num'] || empty($num)) {
                // 积分赠送
                $k_re=$this->give_point($fromusername, $info);				
                $articles['content'] = '签到成功（银币、贡献值各增加：5）';				
            } else {
                $articles['content'] = '今日签到次数已用完，请明日再来！';
            }
        } else {
    ...
    public function give_point($fromusername, $info)
    {
        if (! empty($info)) {
            ...
            if (isset($config['point_status']) && $config['point_status'] == 1) {
                ...
                // 当前时间减去时间间隔得到的历史时间之后赠送的次数
                if ($num < $config['point_num'] || empty($num)) {
                    $this->do_point2($fromusername, $info, $config['point_value'],$config['point_value']);					
                }
            }
        }
    }
