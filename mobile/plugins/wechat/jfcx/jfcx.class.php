...
plugin_name = $name;
    ...
    {
        $articles = array('type'=>'text', 'content'=>'暂无信息');
        $uid = model('Base')->model->table('wechat_user')->field('ect_uid')->where('openid = "'.$fromusername.'"')->getOne();
        if(!empty($uid)){
            $data = model('Base')->model->table('users')->field('rank_points, pay_points, user_money, gold_coin')->where('user_id = '.$uid)->find();
            if (! empty($data)) {
                $articles['content'] = '现金余额：'.price_format($data['user_money'], false)."\r\n".'奖励红包：'.$data['gold_coin']."\r\n".'贡献值：'.$data['rank_points']."\r\n".'银币：'.$data['pay_points'];
                // 积分赠送
