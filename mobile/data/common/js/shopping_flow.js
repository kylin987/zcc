...
function changeIntegralResponse(obj)
{
  if (obj.error)
  {
    try
    {
      ...
      document.getElementById('ECS_INTEGRAL').value = obj.vau;
 ...
 * 改变金币
 */
function changeGoldcoin(val)
{  	
  if (selectedGoldcoin == val)
  {
    return;
  }
  else
  {
    selectedGoldcoin = val;
  }
  $.get('index.php?m=default&c=flow&a=change_goldcoin', {goldcoin:val}, function(data){
    changeGoldcoinResponse(data);
  }, 'json');
  //Ajax.call('flow.php?step=change_goldcoin', 'goldcoin=' + val, changeGoldcoinResponse, 'GET', 'JSON');
}

/* *
 * 改变金币回调函数
 */
function changeGoldcoinResponse(obj)
{
  ...
  if (obj.error)
  {
    try
    {
      document.getElementById('ECS_GOLDCOIN_NOTICE').innerHTML = obj.error;
      document.getElementById('ECS_GOLDCOIN').value = '0';
      document.getElementById('ECS_GOLDCOIN').focus();
    }
    catch (ex) { }
  }
  else
  {
    try
    {
      document.getElementById('ECS_GOLDCOIN_NOTICE').innerHTML = '';
    }
    catch (ex) { }
    orderSelectedResponse(obj.content);
  }
...
var ECS_UPID ='';
	//检查用户是否输入推荐码
	var upid = document.getElementById("ECS_UPID").value;
	if(upid=='' || upid==undefined || upid==null){
		/*
		if(window.confirm(flow_no_upid)){
